...
def main():
    ...
    with tab1:
        ...
        with col1:
            ...
            weight = st.number_input("Weight (kg).", min_value=1, max_value=150, step=1)
            height = st.number_input("Height (cm).", min_value=80, max_value=230, step=1)
    ...
    # Tab 3: Combined Calorie-Based Weekly Menu Generator
    with tab3:
        st.subheader("Calorie-Based Weekly Menu Generator")

        # Step 1: Calorie Calculation (from Tab 1)
        ...
        # Step 2: Weekly Menu Input (from Tab 2 with modifications)
        ...
        # Multiday dishes option
        multiday_dishes = st.radio(
            "Are multiday dishes allowed (same dish for multiple days)?",
            options=["Yes", "No"],
            index=0
        )
        ...
        if st.button("Generate Calorie-Based Weekly Menu"):
            ...
            aggregated_prompt = (
                ...
                f"The three meals together must make up for a total of {necessary_calories} calories. Not necessarily each meal must be the same amount of calories, if the total adds up.\n"
            ...
            response = get_openai_response([{"role": "user", "content": aggregated_prompt}])
            st.markdown("### Generated Menu:")
            st.markdown(response)

            # Parse and display the menu and shopping list
            menu_data, shopping_list_data = parse_menu_and_shopping_list(response)
